...
## Universal Editor (Crosswalk) HTML Serialization Rules
...
The UE serializes each field from the block's model (`_*.json`) into HTML following these rules:

#### Standalone blocks (blocks with a `model` property in the template)

Each field in the model becomes a **separate row** in the block HTML:

| Model field type | HTML output |
|---|---|
| `reference` (image) + `text` (alt) | `<div><div><picture>...<img alt="Alt Text">...</picture></div></div>` — image and alt are combined into a single row |
| `text` | `<div><div>plain text value</div></div>` — plain text in a div, **no semantic HTML** |
| `richtext` | `<div><div><p>...</p><h2>...</h2></div></div>` — HTML content preserved (headings, bold, italic, links) |
| Empty/unfilled field | `<div></div>` — empty row |

**Key insight:** `reference` + `text` (imageAlt) pairs are grouped into a single row. Other fields each get their own row.
...
<div class="hero">
  <div>
    <div><picture>...<img alt="Alt value">...</picture></div>
  </div>
  <div>
...
```json
{
  "fields": [
    ...
    { "component": "richtext", "name": "description" }
  ]
}
```
...
#### Collection blocks (blocks with a `filter` and child items)

Each **item** becomes a row, and each field within the item becomes a **column** within that row:

**Example — Cards model (each card has image + richtext):**
```html
<div class="cards">
  <div>
...
### Writing local .plain.html to match UE output

When creating test content for crosswalk projects:

1. **Read the block's model** (`blocks/{block-name}/_{block-name}.json`)
2. **Apply the serialization rules above** to generate the correct HTML structure
3. **For standalone blocks:** each field = separate row, image+alt grouped
4. **For collection blocks:** each item = row, fields = columns within the row
5. **Use `<div>` without semantic HTML** for `text` fields (the JS decoration should create `<h2>`, `<p>`, etc.)
6. **Preserve HTML** for `richtext` fields (headings, paragraphs, bold, etc.)

### Decoration implications

Because UE serializes `text` fields as plain `<div>` content (no semantic HTML), the block's JavaScript decoration (`demo.js`) is responsible for:
